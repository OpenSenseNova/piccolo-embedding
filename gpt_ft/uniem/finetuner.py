...
# from uniem.data import M3EDataset
from datasets import Dataset
from uniem.data import MixFinetuneDataset

class MixFineTuner(FineTuner):
    def __init__(
        self,
        model_name_or_path: str,
        ...
        self.model_name_or_path = model_name_or_path
        self.raw_dataset = dataset
        self.record_type = RecordType.SCORED_PAIR # hard code here !!!
        self.tokenizer = AutoTokenizer.from_pretrained(self.model_name_or_path)
    
    def create_finetune_datasets(
        self,
    ) -> tuple[MixFinetuneDataset, MixFinetuneDataset | None]:
        train_dataset = MixFinetuneDataset(self.raw_dataset, record_type=self.record_type, split='train')
        validation_dataset = MixFinetuneDataset(self.raw_dataset, record_type=self.record_type, split='validation')
        return train_dataset, validation_dataset