...
class Trainer:
    def __init__(
        ...
        *,
        ...
        self.current_step = 0
        self.ckpt_dir = os.path.join(output_dir, 'ckpts')
    ...
    def train(self):
        for current_epoch in range(1, self.epochs + 1):
            ...
            if current_epoch % self.save_freq == 0:
                ...
                unwrapped_model = cast(EmbedderForTrain, self.accelerator.unwrap_model(self.model))
                output_dir = os.path.join(self.ckpt_dir, 'e_{}'.format(current_epoch))
                self.accelerator.print('save epoch {} ckpt to {}'.format(current_epoch, output_dir))
                os.makedirs(output_dir, exist_ok=True)
                unwrapped_model.embedder.save_pretrained(output_dir)
